...
config-builder is "yet another config utility" (or YACF, if you like acronyms). Its purpose is to build 
...
config-builder has a practically unlimited set of other "configurationy" uses too! Use it to build an app
...
If you're running via an npm script, then you can list config-builder as a `devDependency` and then call it
...
tedious. But with config-builder, we can replace `prod` in our base config with a property reference, 
